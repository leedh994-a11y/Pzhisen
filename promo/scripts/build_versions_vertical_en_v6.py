#!/usr/bin/env python3
"""Build NEW 5 vertical EN promo MP4s — pack v6 (unique presenters/scripts/UI vs prior packs)."""
...
VERSION_META = [
    ...
    },
    {
        ...
        "drift_cycle": ["left", "center", "up", "left", "center", "up", "left", "center", "up", "left", "center", "left"],
        "zoom_alt": False,
        "zoom_speed": 0.00095,
    },
    {
    ...
    },
    {
        ...
        "subtitle_fs": 38,
        "subtitle_margin_v": 176,
        "subtitle_primary": "&H00FEF2F2",
    ...
    },
    {
        ...
        "subtitle_fs": 34,
        "subtitle_margin_v": 122,
        "subtitle_primary": "&H00F0FDFA",
        "drift_cycle": ["right", "up", "center", "left", "up", "center", "right", "up", "left", "center", "up", "center"],
        "zoom_alt": True,
        "zoom_speed": 0.00120,
    },
]
...
def build_one(meta: dict) -> Path:
    ...
    outfile = OUT_DIR / f"pzhisen-promo-en6-{vid}-vertical.mp4"

    scenes = scene_plan(duration, ver_dir, meta)
    print(f"\n=== Building v6 {vid} ({meta['name']}) {duration:.1f}s vertical ===")

    drifts = meta["drift_cycle"]
    zoom_alt = meta["zoom_alt"]
    zoom_speed = meta["zoom_speed"]

    with tempfile.TemporaryDirectory(prefix=f"pzhisen-en6-{vid}-") as tmp:
        tmp_path = Path(tmp)
        clips: list[Path] = []
...
def main() -> None:
    ...
    for meta in VERSION_META:
        if only != "all" and only != meta["id"]:
            continue
        build_one(meta)
    print("\nAll v6 unique vertical videos built.")
